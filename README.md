...
백엔드 서버는 **AWS Elastic Beanstalk를 통해 자동 배포**되며,  
데이터베이스는 **AWS RDS(MySQL)** 를 사용하고, 이미지 저장은 **S3**에 연동했습니다.

Swagger UI를 통해 직접 API를 호출해볼 수 있습니다.  
프론트엔드는 별도로 배포하지 않았고 **시연 영상에서 React 구현 내용을 확인할 수 있습니다.**
...
* **AWS Elastic Beanstalk + RDS + S3로 백엔드 서버 배포 경험**
...
이러한 방식으로 사용자 입장에서 자연스러운 채팅을 구현했습니다.
